...
void UR10Controller::setup(string ipAddress, RobotParameters & params){
    ...
    movement.setup();
    for(int i = 0; i < 8; i++){
        UR10KinematicModel * foo = new UR10KinematicModel();
        foo->setup();
...
vector<double> UR10Controller::getCurrentPose(){
    return robot.getCurrentPose();
}

//------------------------------------------------------------------
vector< double > UR10Controller::getArmIK( float aDeltaTimef ) {
...
//------------------------------------------------------------------
vector< double > UR10Controller::getArmIK( ofVec3f aTargetWorldPos, ofVec3f aElbowWorldPos, bool aBInvertElbow, float aDeltaTimef ) {
...
//------------------------------------------------------------------
ofVec3f UR10Controller::getYawPitchRoll( ofQuaternion aquat ) {
    float qx = aquat.x();
    float qy = aquat.y();
    float qz = aquat.z();
    float qw = aquat.w();
...
//------------------------------------------------------------------
float UR10Controller::getNeckAngleAlignedWithVector( ofVec3f avec ) {
...
//------------------------------------------------------------------
ofVec3f UR10Controller::lerp( ofVec3f aStartVec, ofVec3f aEndVec, float aLerpAmnt ) {
    ofVec3f tmp;
...
//--------------------------------------------------
float UR10Controller::lerpRadians(float currentAngle, float targetAngle, float pct, float alerp ) {
...
//------------------------------------------------------------------
ofVec3f UR10Controller::getIKRobotTargetForWorldPos( ofVec3f aWorldTarget, bool bRepel ) {
...
//------------------------------------------------------------------
vector<double> UR10Controller::lookAtJoints( float aDeltaTimef ) {
...
//------------------------------------------------------------------
float UR10Controller::getZValueForIkRobotLocalY( float aLocalY, float aWorldZ ) {
...
void UR10Controller::updateRobotData(){
...
#pragma mark - drawing
void UR10Controller::close(){
    if(robot.isThreadRunning()){
        robot.stopThread();
    }
}

void UR10Controller::draw(bool debug){
    actualArm.draw(debug);
}

void UR10Controller::drawPreviews(){
...
void UR10Controller::drawIK(){
    if(mIKArm) mIKArm->draw();
    if(mIKArmInverted)mIKArmInverted->draw();
}

void UR10Controller::drawPreview(){
    previewArm->draw();
    
}

void UR10Controller::enableControlJointsExternally() {
    m_bSettingJointsExternally = true;
}

void UR10Controller::disableControlJointsExternally() {
    m_bSettingJointsExternally = false;
}

bool UR10Controller::areJointsControlledExternally() {
