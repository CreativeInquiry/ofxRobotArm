...
public:
    ...
    void standard_ik(int num_chains){
    ...
    void fn relaxed_ik(num_chains: usize){
         let mut objectives: Vec<Box<dyn ObjectiveTrait>> = Vec::new();
         let mut weight_priors: Vec<f64> = Vec::new();
         for i in 0..num_chains {
             objectives.push(Box::new(MatchEEPosGoals::new(i)));
     ...
     double fn call(&self, x: &[f64], vars: &RelaxedIKVars){
         if self.lite {
     ...
     gradientReturn fn gradient(&self, x: &[f64], vars: &RelaxedIKVars) {
         if self.lite {
    ...
    int num_chains,
    vector<double> weight_priors;
    bool lite;
    bool finite_diff_grad;
};
}
}
