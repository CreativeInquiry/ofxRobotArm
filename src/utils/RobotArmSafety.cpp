...
void RobotArmSafety::update( float aDeltaTimef ) {
    ...
    for( int i = 0; i < mDesiredAngles.size(); i++ ) {
        ...
        //            //            }
        //            //
        ////            cout << i << " targetAngle: " << mTargetRobotAngles[i] << " cAngle: " << mCurrentRobotArmAngles[i] << " lerpAngle: " << lerpAngle << " diff: " << tdiff << " epsilon: " << tEpsilon << " | " << ofGetFrameNum() << endl;
        //            //
...
void RobotArmSafety::checkCollision(float aDeltaTimef){
    if( mCollision->shouldApply() ) {
        mCollision->setRobotAngles(mCurrentRobotArmAngles);
        mCollision->setDesiredAngles( mTargetRobotAngles );
        mCollision->update( aDeltaTimef );
        mDesiredAngles = mCollision->getDesiredAngles();
    }
